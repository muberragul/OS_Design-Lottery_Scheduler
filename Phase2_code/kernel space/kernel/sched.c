...
#include <linux/changeflag.h> // ramo changing flag header file
#include <linux/random.h>

extern void timer_bh(void);
extern void tqueue_bh(void);
extern void immediate_bh(void);
extern int flag; //ramo changing flag value
/*
 * scheduler variables
 */
// ramo changing
#define MAX_USERS 10
#define MAX_PROCESSES_PER_USER 10
...
repeat_schedule:
    ...
    if(flag==0)
    {
        ...
        */
        ...
        // Loterry scheduling algorithm
        struct user_info *users[MAX_USERS];

        int user_count = 0;
        //repeat schedule:
        //userlar için process sayısı bulmak
		int i;
        ...
        list_for_each(tmp, &runqueue_head) {
            pid_t p_uid;
            p = list_entry(tmp, struct task_struct, run_list);
            if (can_schedule(p, this_cpu) && p->uid >= 1000) {
                p_uid = p->uid;
                int flagTest = 0;
                for (i = 0; i < user_count; i++){
                    if(p_uid == users[i]->uid) { // eğer eşitse uider count değerini arttır
                ...
                if (!flagTest) {
					struct user_info *user = kmalloc(sizeof(struct user_info), GFP_KERNEL); // Bellek tahsisi
						user->uid = p_uid;
        ...
        //ticket değerlerini vericez
                
        int ticket_per_user = 60;
        //int totalTicketNum = ticket_per_user * user_count;
        list_for_each(tmp, &runqueue_head) {
            int flagroot = 0;
            for (i = 0; i < user_count; i++) {
                int ticket_per_process = ticket_per_user / users[i]->process_count; //1001 - 3 process 5 ticket each
                    p = list_entry(tmp, struct task_struct, run_list);
                    if (can_schedule(p, this_cpu)){
                        if(p->uid == users[i]->uid){
                            p->ticket = ticket_per_process;    
                            flagroot = 1;    
                            break;    //ticketı varsa diğer userlar için kontrol etme               
                        }
                    }
            }
            p = list_entry(tmp, struct task_struct, run_list);
            if(flagroot == 0 && can_schedule(p, this_cpu)){
        ...
        if(user_count == 0){
            ...
            int step=0; // her process ticket ı eklemek için
            // lottery değerine göre
            list_for_each(tmp, &runqueue_head) {
                p = list_entry(tmp, struct task_struct, run_list);
                if (can_schedule(p, this_cpu)) {
                    step = step + p->ticket;
